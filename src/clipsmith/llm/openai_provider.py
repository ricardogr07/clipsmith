...
class OpenAIProvider:
    ...
    def pick(
        ...
        with tracer.start_as_current_span(
            "llm.call",
            attributes={"provider": "openai", "model": self._model, "candidate_id": candidate_id},
        ):
            try:
                ...
                llm_call_duration.record(elapsed, {"provider": "openai"})
                LLM_CALL_DURATION.labels(provider="openai").observe(elapsed)
                llm_calls_total.add(
                    1, {"provider": "openai", "outcome": "include" if pick.include else "skip"}
                )
                log.info(
