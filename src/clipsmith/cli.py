...
def setup(
    ...
    if provider == "ollama":
        ...
        # 2. Check if Python client is importable
        try:
        ...
        # 3. Pull the model (shows progress bar in terminal)
        console.print(
            f"[cyan]Pulling model[/cyan] [bold]{ollama_model}[/bold] "
            f"(this may take a while on first run)..."
        )
        result = _sp.run(["ollama", "pull", ollama_model])
...
def check_ollama(
    ...
    # 3. Model available locally
    available = [m.model for m in (models_resp.models or []) if m.model]
    # ollama model names may have a :latest suffix appended automatically
